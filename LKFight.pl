...
# Allez hop, un petit tournoi en passant.
$ua->post("$API_URL/farmer/register-tournament", Content => [token => '$']);

# On parcours les poireaux.
foreach my $leek( keys %{$myLeeks} ) {
  print STDOUT "Combat du poireau $leek pour $login.\n";

  # Tant quՈ faire, on inscrit le poireau aux tournois du jour.
  $ua->post("$API_URL/leek/register-tournament/", Content => [leek_id => $leek, token => '$']);
